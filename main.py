...
INACTIVITY_THRESHOLD_MIN = 50  # Minimum seconds before offline counts
INACTIVITY_THRESHOLD_MAX = 60  # Maximum seconds before offline counts
...
async def timetrack_update():
    ...
    for uid, log in activity_logs.items():
        # Offline timer logic with random delay
    ...
    for uid, log in activity_logs.items():
        if log.get("mute_expires"):
            ...
            if now >= expires:
                ...
                # Clear mute info after automatic unmute
