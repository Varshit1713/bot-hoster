...
LOG_CHANNEL_ID = 1403422664521023648  # The dedicated log channel
...
async def rmute(ctx, member: discord.Member, duration: str, *, reason: str):
    """Mute a member with timeout, role, and log."""
    guild = ctx.guild
...
async def runmute(ctx, member: discord.Member):
    ...
    if muted_role not in member.roles and not log.get("mute_expires"):
